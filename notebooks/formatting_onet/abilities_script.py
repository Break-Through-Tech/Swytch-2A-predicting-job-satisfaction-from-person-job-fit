...
# Load data
df = pd.read_csv("../../data/onet_abilities/abilities.csv")
...
# Flatten the MultiIndex columns into abilities_<element>_<importance OR level> names
wide.columns = [
    f"abilities_{name}_{'importance' if scale == 'IM' else 'level'}"
...
ranked_im.name = "abilities_importance_ranked"
...
ranked_lvl.name = "abilities_level_ranked"
...
final.to_csv("formatted_abilities.csv", index=False)